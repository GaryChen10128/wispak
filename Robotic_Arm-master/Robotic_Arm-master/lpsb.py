...
def trajectory_planner(Q_matrix, time, acceleration, delta_time):
    ...
    for point in range(cartesian_points):
        ...
        straight_vel_mid = [round(x / y, 2) for x, y in zip(delta_position_array[1:-1], time[1:-1])]
        ...
        straight_vel_time = time[0] - 0.5 * t_blend_array[0]

        straight_vel_array.append(delta_position_array[0] / (straight_vel_time))
        ...
        straight_vel_array.append(delta_position_array[-1] / (time[-1] - 0.5 * t_blend_last))
        ...
        # t_straight
        t_straight_array.append(round(time[0] - t_blend_array[0] - 0.5 * t_blend_array[2], 1))
        t_straight_mid = [round(time[i] - 0.5 * (t_blend_array[i] + t_blend_array[i + 1]), 1) for i in
                          range(1, len(time) - 1)]
        ...
        for i in range(len(t_blend_array) + len(t_straight_array)):
            k = int(i / 2)  # 0,0,1,1,2,2,3
            if i % 2 == 0:
                interval_next = round(t_blend_array[k], 1) + interval[-1]
                print(i, interval_next)
                interval.append(interval_next)
                # print(k, interval)

                while time_current >= interval[-2] and time_current < interval[-1]:
                    time_current += delta_time
                    acceleration_d.append(acceleration_signed_array[k])
                    omega.append(omega[-1] + acceleration_signed_array[k] * delta_time)
                    theta.append(
                        theta[-1] + omega[-1] * delta_time + 0.5 * acceleration_signed_array[k] * (delta_time ** 2))

            else:
                interval_next = interval[-1] + round(t_straight_array[k], 1)
                if interval_next >= sum(time[:k + 1]):
                    interval.append(sum(time[:k + 1]))
                else:
                    interval.append(interval[-1] + round(t_straight_array[k], 1))
                while time_current >= interval[-2] and time_current < interval[-1]:
                    time_current += delta_time
                    acceleration_d.append(0.)
        ...
        acceleration_all.append(acceleration_d)

    return theta_all, omega_all, acceleration_all
